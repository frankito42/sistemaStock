...
$entradaSql="INSERT INTO `entrada`(`fecha`, `observacion`,`idProve`,idEstablecimiento)VALUES 
                                                                    (:fecha,
...
for ($i=0; $i < count($idArticulo) ; $i++) {
   ...
   if($idArticulo[$i]==0){
       if(isset($vence[$i]) && !empty($vence[$i])){
        
    }else{
        $vence[$i]=NULL;
    }
      $sqlAddNewProduct="INSERT INTO `articulos`(`nombre`, `costo`,cantidad,
    `idEsta`, idProveedor,`mayoritario`,fechaVence,menorCentaje) VALUES 
    (:nombre,:costo,
   :cc, :idEsta,:idP,:mayo,:fechaVence,:meno)";
    $addNewProduct=$conn->prepare($sqlAddNewProduct);
    $addNewProduct->bindParam(":nombre",$_POST['nombre'][$i]);
    $addNewProduct->bindParam(":costo",$costo[$i]);
    
    $addNewProduct->bindParam(":cc",$cantidad[$i]);
    $addNewProduct->bindParam(":idEsta",$esta);
    $addNewProduct->bindParam(":idP",$idProve);
    $addNewProduct->bindParam(":mayo",$precioVenta[$i]);
    $addNewProduct->bindParam(":fechaVence",$vence[$i]);
    $addNewProduct->bindParam(":meno",$menorCentaje[$i]);
    $addNewProduct->execute();
    $idArticulo[$i]=$conn->lastInsertId();
 ...
 /*    $costo[$i]+=$transporte[$i]; */
    ...
    $sqlUpdateStock="UPDATE `articulos` SET nombre=:nombre,`costo`=:costo, `cantidad`=:cantidad, `idProveedor`=:idProveedor, mayoritario=:may, menorCentaje=:menorPorcentaje, fechaVence=:vence WHERE `articulo`=:id";
    $upCantidad=$conn->prepare($sqlUpdateStock);
    $upCantidad->bindParam(":nombre",$_POST['nombre'][$i]);
