...
if(isset($_GET['id']) && !empty($_GET['id'])){
    $idEsta=$_GET['id'];
    $sqlTodosLosArticulos="SELECT pro.nombreP,pro.idProveedor,a.`articulo`, a.`nombre`, a.`costo`, a.`stockmin`, a.`cantidad`, a.`descripcion`, a.`imagen`, a.`categoria`, a.`codBarra`, a.`precioVenta`, a.`idEsta`, e.nombreEsta, c.nombreCategoria,mayoritario,menorCentaje FROM `articulos` as a LEFT JOIN establecimiento=e on a.idEsta=e.idEsta LEFT JOIN proveedores as pro on pro.idProveedor=a.`idProveedor` LEFT JOIN categoria=c on c.idCategoria=a.categoria where a.idEsta=$idEsta";
}else{
